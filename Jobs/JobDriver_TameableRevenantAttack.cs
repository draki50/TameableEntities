...
// RimWorld.JobDriver_TameableRevenantAttack 
using System;
...
{
    public class JobDriver_TameableRevenantAttack : JobDriver
    {
        ...
        protected override IEnumerable<Toil> MakeNewToils()
        {
            ...
            Toil stalk = Toils_Combat.FollowAndMeleeAttack(TargetIndex.A, TargetIndex.B, delegate
            {
                if (!base.pawn.stances.FullBodyBusy)
                {
                    ...
                    Find.Anomaly.Hypnotize(target, base.pawn, LongStunTicks);
            ...
            toil2.tickIntervalAction = (Action<int>)Delegate.Combine(toil2.tickIntervalAction, (Action<int>)delegate (int delta)
            {
                ...
                {
                    Pawn newTarget = GetClosestHostileTarget(10f);

                    if (newTarget != null)
                    {
                        curJob.SetTarget(TargetIndex.A, newTarget);
                        base.pawn.mindState.enemyTarget = newTarget;
                    }
                }
            ...
            toil3.tickAction = (Action)Delegate.Combine(toil3.tickAction, (Action)delegate
            {
                ...
                if (!pawn.stances.stunner.Stunned && Find.TickManager.TicksGame >= hypnotizeEndTick)
                {
                    ...
                    RevenantReflection.BecomeInvisibleTick(Comp) =
        ...
        private Pawn GetClosestHostileTarget(float radius)
        {
